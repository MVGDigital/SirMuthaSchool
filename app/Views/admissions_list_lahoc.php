    ...
    $('#admissionsTable').DataTable({
        ...
        buttons: [{
            ...
            action: function (e, dt, node, config) {
                ...
                // Manually include hidden data (you can customize this)
                var extendedData = data.map(function(row) {
                    // Add hidden columns here, e.g., registration_number or dob
                    row.push(row.registration_number);  // Assuming you want to add registration number
                    row.push(row.dob);  // Similarly for other hidden fields
                    return row;
                });

                // Export the data
                var excelData = new Blob([JSON.stringify(extendedData)], { type: "application/vnd.ms-excel" });
